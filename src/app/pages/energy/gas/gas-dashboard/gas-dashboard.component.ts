...
// ✅ Interface alignée sur le backend C# : { IdSeuil, EnergieId, Valeur, Periode }
export interface Seuil {
  idSeuil:   number;
  energieId: number;
  valeur:    number;   // ✅ Valeur (pas valeurCible)
  periode:   string;   // ✅ Periode (pas annee)
}
...
const ENERGIE_GAZ_ID    = 2;
const TOAST_DURATION_MS = 4_000;
const PAGE_SIZE         = 10;
const TARIF_DT_PAR_M3   = 0.45;

const CHART_W     = 800;
const CHART_H     = 220;
const CHART_PAD_X = 52;
...
export class GasDashboardComponent implements OnInit, OnDestroy {
  ...
  // ── Config énergie ───────────────────────────────
  readonly energieId    = ENERGIE_GAZ_ID;
  readonly energieLabel = 'Gaz';
  ...
  ngOnInit(): void {
    this.canRead  = this.perm.canRead('gaz');
    this.canWrite = this.perm.canWrite('gaz');
    this.initForms();
    this.loadData();
    this.startClock();
  }
  ...
  private initForms(): void {
    ...
    // ✅ FIX : formulaire aligné sur le backend { valeur, periode } — plus de annee/valeurCible
  ...
  loadData(): void {
    ...
    this.api.getMesuresByEnergie(this.energieId)
      ...
      .subscribe({
        next: (data: any[]) => { this.mesures = data.map(m => this.normalizeMesure(m)); done(); },
    ...
    this.api.getSeuilByEnergie(this.energieId)
      ...
      .subscribe({
        next: (data: any) => {
          // ✅ normaliser : objet unique ou tableau
  ...
  get coutTotal(): number {
    return this.round(this.totalConsommation * this.tarifGaz, 2);
  }

  get economiePotentielle(): number {
    if (!this.seuilAlerte || this.totalConsommation <= this.seuilAlerte) return 0;
    return this.round((this.totalConsommation - this.seuilAlerte) * this.tarifGaz, 2);
  }
  ...
  // ✅ FIX : utilise s.valeur (pas s.valeurCible)
  ...
  buildChart(): void {
    ...
    if (!filtered.length) {
      ...
      this.linePath = ''; this.areaPath = ''; return;
    }
    ...
    if (this.chartPoints.length) {
      ...
      this.areaPath = `M ${pts[0].x},${bottom} ` + pts.map(p => `L ${p.x},${p.y}`).join(' ') + ` L ${pts[pts.length - 1].x},${bottom} Z`;
    }
  ...
  // ✅ FIX : utilise s.valeur (pas s.valeurCible / s.valeurActuelle)
  ...
  openAddSeuilModal(): void {
    ...
    // ✅ FIX : reset avec les bons champs (valeur, periode) — plus de annee/valeurCible
    this.seuilForm.reset({ periode: 'Mensuel', valeur: '' });
    this.showSeuilModal = true;
  }

  // ✅ FIX : patch avec s.valeur (pas s.valeurCible)
  ...
  /**
   * ✅ FIX PRINCIPAL :
   *   - Payload envoyé : { energieId, valeur, periode } — aligné sur le backend C#
   *   - Mode édition  → PUT  /seuil/energie/{energieId}
   *   - Mode création → upsertSeuil() (GET d'abord → POST ou PUT selon existence)
   *     Évite le 409/500 quand un seuil existe déjà pour cet energieId
   */
  saveSeuil(): void {
    ...
    const payload = {
      energieId: this.energieId,
      valeur:    +formVal.valeur,   // ✅ cast en number, nom backend
    ...
    obs.pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: (err: any) => {
        ...
        console.error('[saveSeuil gaz]', err);
      },
    });
  }
  ...
  // ✅ FIX : suppression réelle côté backend via deleteSeuilByEnergie
  deleteSeuil(): void {
  ...
  saveAlerte(): void {
    ...
    obs.pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: () => { this.alerteSaving = false; this.showToast('Erreur lors de l\'enregistrement.', 'error'); },
    });
  }
  ...
  exportAlertes(): void {
    ...
    this.downloadFile(header + rows, 'alertes-gaz.csv', 'text/csv');
  }
  ...
  openEditMesure(m: Mesure): void {
    ...
    this.mesureForm.patchValue({
      valeur: m.valeur, dateMesure: m.dateMesure?.slice(0, 16) ?? this.nowIso(),
      sourceDonnee: m.sourceDonnee, energieId: m.energieId,
      equipementId: m.equipementId ?? '', commentaire: m.commentaire ?? '',
    });
  ...
  saveMesure(): void {
    ...
    obs.pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: () => { this.mesureSaving = false; this.showToast('Erreur lors de l\'enregistrement.', 'error'); },
    });
  }
  ...
  exportCSV(): void {
    ...
    this.downloadFile(header + rows, 'mesures-gaz.csv', 'text/csv');
  }
  ...
  lancerAnalyseIA(): void {
    ...
    this.api.ollamaAnalyse({
      ...
      next:  (res: any) => { this.iaLoading = false; this.iaAnalyse = res.response; },
      error: () => {
        ...
        this.iaAnalyse =
          ...
          (this.anomaliesCount > 0 ? `⚠️ ${this.anomaliesCount} anomalie(s) détectée(s).` : `✅ Consommation dans les normes.`);
      },
    });
  }
  ...
  envoyerIaChat(): void {
    ...
    const prompt =
      `Contexte gaz WICMIC — Total : ${this.totalConsommation} m³, ` +
  ...
  computeComparaison(): void {
    ...
    const stat = (range: { debut: Date; fin: Date; label: string }): StatPeriode => {
      ...
      return { label: range.label, total, nbMesures: ms.length, cout: this.round(total * this.tarifGaz, 2) };
    };
  ...
  telechargerRapport(type: string): void {
    ...
    switch (type) {
      case 'mensuel': {
        const txt = [`RAPPORT MENSUEL GAZ — ${date}`, '='.repeat(50), `Consommation totale  : ${this.totalConsommation} m³`, `Mesures ce mois      : ${this.mesuresMoisCourant}`, `Coût estimé          : ${this.coutTotal} DT`, `Tendance             : ${this.tendanceLabel}`, `Anomalies actives    : ${this.anomaliesCount}`].join('\n');
        this.downloadFile(txt, 'rapport-mensuel-gaz.txt', 'text/plain'); break;
      }
      case 'csv':     this.exportCSV(); break;
      case 'alertes': this.exportAlertes(); break;
      case 'anomalies': {
        const txt = [`RAPPORT ANOMALIES GAZ — ${date}`, '='.repeat(50), ...this.anomaliesLocales.map(a => `[${a.resolu ? 'RÉSOLU' : 'ACTIF'}] ${a.description} | ${a.valeur} m³`)].join('\n');
        this.downloadFile(txt, 'rapport-anomalies-gaz.txt', 'text/plain'); break;
      }
      case 'recommandations': {
        const txt = [`RAPPORT RECOMMANDATIONS GAZ — ${date}`, '='.repeat(50), ...this.recommandations.map(r => `[${r.applique ? 'APPLIQUÉ' : 'EN ATTENTE'}] (${r.priorite}) ${r.texte} — Économie : ${r.economieEstimee ?? '—'} DT`)].join('\n');
        this.downloadFile(txt, 'rapport-recommandations-gaz.txt', 'text/plain'); break;
      }
      default: {
        const txt = [`RAPPORT COMPLET GAZ — ${date}`, '='.repeat(50), `Total mesures        : ${this.totalMesures}`, `Consommation totale  : ${this.totalConsommation} m³`, `Moyenne              : ${this.moyenneMesures} m³`, `Coût total estimé    : ${this.coutTotal} DT`, `Tendance             : ${this.tendanceLabel}`, `Anomalies actives    : ${this.anomaliesCount}`, `Alertes actives      : ${this.alertesActives}`, `Économie potentielle : ${this.economiePotentielle} DT`].join('\n');
        this.downloadFile(txt, 'rapport-complet-gaz.txt', 'text/plain');
      }
    }
  ...
  getSeveriteClass(severite: string): Record<string, boolean> {
    return { 'badge--danger': severite === 'Critique', 'badge--warn': severite === 'Haute', 'badge--ghost': severite === 'Normale' || !severite };
  }

  getPrioriteClass(priorite: string): Record<string, boolean> {
    return { 'badge--danger': priorite === 'Haute', 'badge--warn': priorite === 'Moyenne', 'badge--ghost': priorite === 'Faible' || !priorite };
  }
  ...
  // ✅ FIX : normalisation — gère camelCase et PascalCase du backend
  private normalizeSeuil(s: any): Seuil {
    return {
      idSeuil:   s.idSeuil   ?? s.IdSeuil   ?? 0,
      energieId: s.energieId ?? s.EnergieId ?? 0,
      valeur:    +(s.valeur  ?? s.Valeur    ?? s.valeurCible ?? s.ValeurCible ?? 0),
